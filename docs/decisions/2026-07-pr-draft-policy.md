...
문제는 손이 한 번 더 가는 게 아니라 **PR이 생긴 것을 인지하지 못한다**는 점이었다.
draft는 리뷰 요청 알림을 보내지 않는 것이 설계 목적이다. 관리자는 PR 생성 알림으로
에이전트 작업이 끝난 것을 파악해 왔는데, Claude Code agent를 쓰기 시작한 뒤로 그 알림이
끊기면서 PR이 열린 줄 모르고 방치되는 일이 생겼다. 알림이 인지 경로 자체였던 것이다.

즉 이것은 PR 스타일 선호가 아니라, 워크플로에서 기능 하나가 조용히 사라진 문제다.
특정 레포의 컨벤션이 아니라 **이 하네스를 쓰는 사람의 작업 관리 방식**에 걸린 문제이므로,
레포나 계정으로 범위를 나누지 않고 전역에 적용한다.
